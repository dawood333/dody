...
static void fts_fps_recovery(struct fts_ts_data *ts_data)
{
	u8 cmd = ts_data->last_fps_cmd;

	if (!cmd) {
		cmd = 0x3c;
		FTS_INFO("last fps is 0, reovery to default fps");
	}
...
static void fts_report_rate_480(struct fts_ts_data *ts_data) {
	if (!fts_write_reg(0x88, 0x30))
		FTS_INFO("config report rate as 480 successful");
}
#endif
...
{
	FTS_DEBUG("tp reset");
	fts_data->update_fps &= ~FPS_IC_READY;
...
static int fts_pinctrl_init(struct fts_ts_data *ts)
{
    ...
    ts->pins_spimode = pinctrl_lookup_state(ts->pinctrl, "ft3658u_spi_mode");
	if (IS_ERR_OR_NULL(ts->pins_spimode)) {
		FTS_ERROR("Pin state[spi_mode] not found");
		ret = PTR_ERR(ts->pins_spimode);
		goto err_pinctrl_lookup;
	}
...
static int fts_pinctrl_select_spimode(struct fts_ts_data *ts)
{
	...
	if (ts->pinctrl && ts->pins_spimode) {
		ret = pinctrl_select_state(ts->pinctrl, ts->pins_spimode);
		if (ret < 0) {
			FTS_ERROR("Set spimode pin state error:%d", ret);
		}
	}
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
	...
	FTS_DEBUG("set normal and spimode done, pls check state");
#endif
...
static bool send_fps_cmd(struct fts_ts_data *ts_data)
{

	FTS_DEBUG("suspend = %d, update_fps = %#x", ts_data->suspended, ts_data->update_fps);

	if (!ts_data->suspended)
		return true;

	if (ts_data->update_fps == FPS_UPDATE)
		return true;
	else
		return false;
}

static void fts_fps_work(struct work_struct *work)
{
	struct fts_ts_data *ts_data = container_of(work, struct fts_ts_data,
								  fps_work);
	u8 cmd = 0;

	atomic_set(&ts_data->fps_worker_waiting, true);
	FTS_INFO("wait fps update condition ready...");
	wait_event_interruptible(ts_data->wait_update, send_fps_cmd(ts_data));
	atomic_set(&ts_data->fps_worker_waiting, false);

	cmd = ts_data->fps_cmd;
	ts_data->fps_cmd = 0;
	ts_data->update_fps &= ~FPS_CMD_UPDATE;

	if (!fts_write_reg(0x8a, cmd))
		FTS_INFO("write %#x to 8A successful", cmd);
}
...
static int check_fps(unsigned long event, void *data)
{
	...
	if (CHANGE_FPS == event) {
		...
		if (legally) {
			...
			fts_data->update_fps |= FPS_CMD_UPDATE;

			FTS_INFO("config touch work with %d fps", fps);
			if (suspend && !gesture_en) {
				FTS_INFO("tp sleep, config is put-off to resume");
			} else {
				if (!atomic_read(&fts_data->fps_worker_waiting))
					schedule_work(&fts_data->fps_work);
				else
					FTS_INFO("fps worker is waiting, just update fps cmd");

				wake_up(&fts_data->wait_update);
			}
		}
		return 1;
	} else {
		...
		unsigned long event, void *data)
{
	...
	} else if (event == MI_DISP_DPMS_EVENT && (blank == MI_DISP_DPMS_POWERDOWN ||
...
#ifdef GAME_480
static void fts_set_report_rate(struct fts_ts_data *ts_data)
{
	u8 cmd;
	...
	if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][GET_CUR_VALUE] !=
		...
		if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][SET_CUR_VALUE] == 1) {
			cmd = 0x30;
			FTS_INFO("entry game mode, 480Hz report");
		} else {
			cmd = 0x18;
			FTS_INFO("quite game mode, 240Hz report");
		}

		if (!fts_write_reg(0x88, cmd))
			FTS_INFO("config report rate successful");
	}
}
#endif
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	atomic_set(&ts_data->fps_worker_waiting, false);
	ts_data->update_fps = FPS_IC_READY;
	ts_data->fps_cmd = 0;
	init_waitqueue_head(&ts_data->wait_update);
	INIT_WORK(&ts_data->fps_work, fts_fps_work);
...
static int fts_ts_resume(struct device *dev)
{
	...
	if (ts_data->fps_cmd) {
		if (!fts_write_reg(0x8a, ts_data->fps_cmd))
			FTS_INFO("write %#x to 8A successful", ts_data->fps_cmd);
		ts_data->fps_cmd = 0;
	}
